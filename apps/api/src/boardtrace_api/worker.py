...
async def _run_noop(payload: AnalysisTaskPayload, worker_id: str) -> str:
    ...
    try:
        ...
        try:
            _wait_for_test_gate()
            # Deterministic orchestration-only handler. It deliberately produces no analysis data.
            async with session_factory() as session:
                repository = AnalysisJobRepository(session)
                ...
                if completed:
                    ...
                    metric_observe(metrics, "analysis_job_duration_seconds", 0.0)
                return "completed" if completed else "completion_rejected"
...
def run_analysis_job(task: Task[..., str], payload: dict[str, object]) -> str:
    ...
    return asyncio.run(_run_noop(validated, hostname))
